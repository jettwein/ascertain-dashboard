...
This guide covers the one-time setup needed to use the agentic coding workflow. Once you've completed these steps, you'll be able to use Claude Code across all your projects.

## What is the Agentic Coding Workflow?

This framework uses Claude Code (Anthropic's AI coding assistant) to accelerate development. The workflow includes:
...
- **GitHub integration** (optional) — Automatic code reviews on PRs, and `@claude` mentions for PR feedback
- **Jira integration** (optional) — Claude can read tickets, update statuses, and track work

This setup takes about 10-15 minutes. Steps 2 and 3 (GitHub CLI and Jira CLI) are optional depending on which integrations your project uses.
...
- Access to your GitHub organization (if using GitHub integration)
- Access to your Jira instance (if using Jira integration)
...
## Step 2: Set Up GitHub CLI (Optional)

The GitHub CLI (`gh`) allows Claude to create branches, PRs, and interact with GitHub. Skip this step if your project doesn't use GitHub.
...
## Step 3: Set Up Jira CLI (Optional)

The Jira CLI (`jira`) allows Claude to read tickets, update statuses, and track work locally. Skip this step if your project doesn't use Jira.
...
Run the initialization command with your Jira server:

```bash
jira init --installation cloud --server https://your-org.atlassian.net --login your-email@your-company.com
```

Replace the server URL and email with your actual Atlassian instance and account email.
...
# Jira CLI (if installed)
jira project list | head -3
```

Claude Code must succeed. The others only need to pass if you installed them.
...
git clone https://github.com/jettwein/shipwright.git ~/repos/shipwright

# Symlink the adopt skill to your global skills
ln -sf ~/repos/shipwright/.claude/skills/adopt/SKILL.md ~/.claude/skills/adopt/SKILL.md
```

Now `/adopt` is available in every project you work on, even repos that don't have the framework set up yet.

> **Note:** Using a symlink means the command stays up to date. Just run `git pull` in `~/repos/shipwright` to get the latest version.
...
If the repo already has `.claude/skills/` set up:
...
2. Make sure you're using your Atlassian account email
...
- **Workflow questions** — Ask in your team's Slack channel
...
- Review the skills in `.claude/skills/` to see what's available
